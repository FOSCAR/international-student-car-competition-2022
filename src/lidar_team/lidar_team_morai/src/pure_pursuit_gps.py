...
def gps_callback(data):
    ...
    br = tf.TransformBroadcaster()
    br.sendTransform((gps_msg.pose.position.x, gps_msg.pose.position.y, gps_msg.pose.position.z),
                    tf.transformations.quaternion_from_euler(0, 0, gps_msg.pose.orientation.w/180*pi),
                    rospy.Time.now(),
                    "base_link",
                    "map")
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if gps_msg != None:
            ...
            state = State(x = gps_msg.pose.position.x, y = gps_msg.pose.position.y, yaw = (gps_msg.pose.orientation.w)/180*pi, v = current_v)
            # count = 0
            # if count/300==1 :
            #     global_path_pub.publish(global_path)
            #     count=0
