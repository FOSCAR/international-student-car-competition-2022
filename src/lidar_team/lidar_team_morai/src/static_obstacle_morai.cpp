...
void dynamicParamCallback(lidar_team_morai::st_hyper_parameter_moraiConfig &config, int32_t level) {
  xMinROI = config.st_xMinROI;
  xMaxROI = config.st_xMaxROI;
  yMinROI = config.st_yMinROI;
  yMaxROI = config.st_yMaxROI;
  zMinROI = config.st_zMinROI;
  zMaxROI = config.st_zMaxROI;

  minPoints = config.st_minPoints;
  epsilon = config.st_epsilon;
  minClusterSize = config.st_minClusterSize;
  maxClusterSize = config.st_maxClusterSize;

  xMinBoundingBox = config.st_xMinBoundingBox;
  xMaxBoundingBox = config.st_xMaxBoundingBox;
  yMinBoundingBox = config.st_yMinBoundingBox;
  yMaxBoundingBox = config.st_yMaxBoundingBox;
  zMinBoundingBox = config.st_zMinBoundingBox;
  zMaxBoundingBox = config.st_zMaxBoundingBox;
}
...
int main(int argc, char **argv) {
  ...
  dynamic_reconfigure::Server<lidar_team_morai::st_hyper_parameter_moraiConfig> server;
  dynamic_reconfigure::Server<lidar_team_morai::st_hyper_parameter_moraiConfig>::CallbackType f;
  ...
  staticObsPosePub = nh.advertise<lidar_team_morai::Boundingbox>("/static_obs_position", 0.001);    
