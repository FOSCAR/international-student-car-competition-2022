...
ros::Publisher deliveryObsClusterPub; //Cluster Publishser
ros::Publisher deliveryObsMarkerPub; //Bounnding Box Visualization Publisher
ros::Publisher deliveryObsPosePub; //Bounding Box Position Publisher
ros::Publisher deliveryObsCropboxPub; //Cropbox Publishser
ros::Publisher deliveryInformationPub;

void deliveryParamCallback(lidar_team_erp42::de_hyper_parameterConfig &config, int32_t level) {
  xMinROI = config.de_xMinROI;
  xMaxROI = config.de_xMaxROI;
  yMinROI = config.de_yMinROI;
  yMaxROI = config.de_yMaxROI;
  zMinROI = config.de_zMinROI;
  zMaxROI = config.de_zMaxROI;

  minPoints = config.de_minPoints;
  epsilon = config.de_epsilon;
  minClusterSize = config.de_minClusterSize;
  maxClusterSize = config.de_maxClusterSize;

  xMinBoundingBox = config.de_xMinBoundingBox;
  xMaxBoundingBox = config.de_xMaxBoundingBox;
  yMinBoundingBox = config.de_yMinBoundingBox;
  yMaxBoundingBox = config.de_yMaxBoundingBox;
  zMinBoundingBox = config.de_zMinBoundingBox;
  zMaxBoundingBox = config.de_zMaxBoundingBox;
}
...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& inputcloud) {
  ...
  //delivery Obstacle Detected Message
  lidar_team_erp42::Delivery information;
  ...
  for (vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++, cluster_id++) {
    ...
    if (BoxArray.markers.size() > 0) {
      ...
      information.x = obstacle_vec[0][0];
      information.y = obstacle_vec[0][1];
      information.z = obstacle_vec[0][2];
      information.angle = atan2(-obstacle_vec[0][1], obstacle_vec[0][0]) * 180 / M_PI;
    }
  ...
  deliveryObsClusterPub.publish(cluster);
  deliveryObsMarkerPub.publish(BoxArray);
  deliveryObsPosePub.publish(BoxPosition);
  deliveryObsCropboxPub.publish(cropbox);
  deliveryInformationPub.publish(information); 
}


int main(int argc, char **argv) {
  ros::init(argc, argv, "delivery_obstacle_detector");
  ros::NodeHandle nh;

  dynamic_reconfigure::Server<lidar_team_erp42::de_hyper_parameterConfig> server;
  dynamic_reconfigure::Server<lidar_team_erp42::de_hyper_parameterConfig>::CallbackType f;

  f = boost::bind(&deliveryParamCallback, _1, _2);
  server.setCallback(f);

  ros::Subscriber rawDataSub = nh.subscribe("/velodyne_points", 1, cloud_cb);  // velodyne_points 토픽 구독. velodyne_points = 라이다 raw data

  deliveryObsClusterPub = nh.advertise<sensor_msgs::PointCloud2>("/delivery_obs_cluster", 0.001);                  
  deliveryObsMarkerPub = nh.advertise<visualization_msgs::MarkerArray>("/delivery_obs_marker", 0.001);  
  deliveryObsPosePub = nh.advertise<lidar_team_erp42::Boundingbox>("/delivery_obs_position", 0.001);    
  deliveryObsCropboxPub = nh.advertise<sensor_msgs::PointCloud2>("/delivery_obs_cropbox", 0.001); 
  deliveryInformationPub = nh.advertise<lidar_team_erp42::Delivery>("/delivery_information", 0.001); 
