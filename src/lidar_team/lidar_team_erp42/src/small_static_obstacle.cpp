...
void dynamicParamCallback(lidar_team_erp42::small_st_hyper_parameterConfig &config, int32_t level) {
  ...
  xMaxROI = config.small_st_xMaxROI;
  yMinROI = config.small_st_yMinROI;
  yMaxROI = config.small_st_yMaxROI;
  zMinROI = config.small_st_zMinROI;
  zMaxROI = config.small_st_zMaxROI;

  minPoints = config.small_st_minPoints;
  epsilon = config.small_st_epsilon;
  minClusterSize = config.small_st_minClusterSize;
  maxClusterSize = config.small_st_maxClusterSize;

  xMinBoundingBox = config.small_st_xMinBoundingBox;
  xMaxBoundingBox = config.small_st_xMaxBoundingBox;
  yMinBoundingBox = config.small_st_yMinBoundingBox;
  yMaxBoundingBox = config.small_st_yMaxBoundingBox;
  zMinBoundingBox = config.small_st_zMinBoundingBox;
  zMaxBoundingBox = config.small_st_zMaxBoundingBox;
}
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "small_static_obstacle_detector");
  ros::NodeHandle nh;

  dynamic_reconfigure::Server<lidar_team_erp42::small_st_hyper_parameterConfig> server;
  dynamic_reconfigure::Server<lidar_team_erp42::small_st_hyper_parameterConfig>::CallbackType f;
