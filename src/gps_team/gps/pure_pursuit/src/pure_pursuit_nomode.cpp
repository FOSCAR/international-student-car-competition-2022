...
void PurePursuitNode::run(char** argv) {
  ...
  while (ros::ok()) {
    ...
    // 신호등, 미션 전 직진
    if(pp_.mode == 0 || pp_.mode == 2 || pp_.mode == 4 || pp_.mode == 6 || pp_.mode == 24 || pp_.mode == 26 || pp_.mode == 28){
      pp_.mission_flag = 0;
    ...
    if(pp_.mode == 11 || pp_.mode == 25 || pp_.mode == 22){
      pp_.mission_flag = 0;
      const_lookahead_distance_ = 6;
      //const_velocity_ = 13;
      const_velocity_ = 8;
    ...
    // Normal 커브
    if (pp_.mode == 18 || pp_.mode == 21 || pp_.mode == 23) {
      pp_.mission_flag = 0;
      const_lookahead_distance_ = 4;
      //const_velocity_ = 11;
      const_velocity_ = 6;
    ...
    // MODE 3,5,8,25 : 신호등 (직진구간)
    if (pp_.mode == 3 || pp_.mode == 5 || pp_.mode == 8 || pp_.mode == 25) {
      pp_.mission_flag = 0;
    ...
    if (pp_.mode == 10) {
      ...
      if(pp_.mission_flag == 1){
        const_velocity_ = 4; // if not calculated a_max_index
        }
    }
...
void PurePursuitNode::callbackFromDelivery(const vision_distance::DeliveryArray& msg){
  ...
  // B Area
  if (pp_.mode == 14 && (pp_.mission_flag == 1 || pp_.mission_flag == 2 || pp_.mission_flag == 3)){
...
void PurePursuitNode::callbackFromTrafficLight(const darknet_ros_msgs::BoundingBoxes& msg) {
  ...
  for(int i=0; i<yoloObjects.size(); i++){
    ...
    // if(pp_.mode == 14 && (pp_.mission_flag == 0 || pp_.mission_flag == 22 || pp_.mission_flag == 33)){
  ...
  // if(pp_.mode == 14 && deliveryObjectsB.size() > 0 && (pp_.mission_flag == 1 || pp_.mission_flag == 2 || pp_.mission_flag == 3)){
