...
int slow_down_tf_idx_8 = 1000;
int slow_down_tf_idx_9 = 1000;
...
void PurePursuitNode::run(char** argv) {
  ...
  while (ros::ok()) {
    ...
    if(!index_flag){
      ...
      slow_down_tf_idx_8 = pp_.getPosIndex(slow_down_tf_coord8[0] , slow_down_tf_coord8[1]);
      slow_down_tf_idx_9 = pp_.getPosIndex(slow_down_tf_coord9[0] , slow_down_tf_coord9[1]);
    ...
    if (pp_.mode == 3 || pp_.mode == 5 || pp_.mode == 8 || pp_.mode == 33) {
      ...
      else if((pp_.reachMissionIdx(slow_down_tf_idx_1) || pp_.reachMissionIdx(slow_down_tf_idx_2) || pp_.reachMissionIdx(slow_down_tf_idx_3) || pp_.reachMissionIdx(slow_down_tf_idx_7)) && pp_.straight_go_flag){
    ...
    if (pp_.mode == 21 || pp_.mode == 23){
      ...
      // When traffic lights are GREEN at slow_down_point -> SPEEDING UP
      else if((pp_.reachMissionIdx(slow_down_tf_idx_4) || pp_.reachMissionIdx(slow_down_tf_idx_5)) && pp_.straight_go_flag){
        while(const_velocity_ < 10){
            const_velocity_ += 0.1;
            pulishControlMsg(const_velocity_ , 0);
            ROS_INFO_STREAM("*****GREEN LIGHT SPEEDING UP*****");
    ...
    if (pp_.mode == 28) {
      ...
      // When traffic lights are GREEN at slow_down_point -> SPEEDING UP
      else if(pp_.reachMissionIdx(slow_down_tf_idx_6) && pp_.straight_go_flag){
        while(const_velocity_ < 10){
            const_velocity_ += 0.1;
            pulishControlMsg(const_velocity_ , 0);
            ROS_INFO_STREAM("*****GREEN LIGHT SPEEDING UP*****");
    ...
    if (pp_.mode == 35 || pp_.mode == 37) {
      ...
      // When traffic lights are GREEN at slow_down_point -> SPEEDING UP
      else if((pp_.reachMissionIdx(slow_down_tf_idx_8) || pp_.reachMissionIdx(slow_down_tf_idx_9))  && pp_.straight_go_flag){
        while(const_velocity_ < 10){
            const_velocity_ += 0.1;
            pulishControlMsg(const_velocity_ , 0);
            ROS_INFO_STREAM("*****GREEN LIGHT SPEEDING UP*****");
